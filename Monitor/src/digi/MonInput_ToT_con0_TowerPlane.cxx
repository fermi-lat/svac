...
//
#include "MonInput_ToT_con0_TowerPlane.h"
...
#define NAME ToT_con0_TowerPlane
#define OUTBRANCH "ToT_con0_TowerPlane"
#define LEAF "ToT_con0_TowerPlane[16][36]/s"
...
int MonInput_ToT_con0_TowerPlane::setOutputBranch(TTree* tree) {
 TBranch* bErr= tree->Branch(OUTBRANCH,m_val,LEAF);
 return bErr != 0 ? 0 : 1;
}
void MonInput_ToT_con0_TowerPlane::enableInputBranch(TTree& tree){
  tree.SetBranchStatus(INBRANCH,1);
}
void MonInput_ToT_con0_TowerPlane::setValue(TObject* event) {
  ...
  for(Int_t i = 0; i < nTkrDigis; i++) {
    ...
    m_val[TowerId][Plane] = tkrDigi->getToT(0);
  }

}
std::string MonInput_ToT_con0_TowerPlane::getInputSource(){
  return INPUTSOURCE;
}
std::string MonInput_ToT_con0_TowerPlane::getDescription(){
  return DESCRIPTION;
}
