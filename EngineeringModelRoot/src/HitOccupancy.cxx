...
int main(int argc, char** argv)
{
  ...
  // read in names of input digi root file and output root file
  if(argc == 3) {
    digiFile = argv[1];
    outputFile = argv[2];
  }
    ...
    digiFile = argv[1];
    outputFile = "hitOccupancy.root";
  }

  TFile f(outputFile.c_str(), "RECREATE");

  // create 3 histograms to save hit occupancy in electronic space
  ...
  // loop through each event in the digi root file
  for(int iEvent = 0; iEvent != nEvent; ++iEvent) { 
	...
	// check which end the strip is read out
	int end;
	if(last0Strip < 0) {
	  end = 1;
	}
	...
	// map from physical space into electronic space
