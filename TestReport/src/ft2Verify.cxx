...
IntervalError::IntervalError(string errName, float errValue)
  : m_errName(errName),
    m_errValue(errValue)
{
}
...
void ft2Verify::analyzeFt2(const char* ft2FileName="ft2.fit")
{
  ...
  for (; ft2file.itor() != ft2file.end(); ft2file.next()) {
    ...
    if (livetime < 0) {
      cout << "Negative livetime at inteval " << interval << endl;
    }
    if (livetime > tdiff) {
      cout << "Livetime is bigger than elapsed time at inteval " << interval << endl;
    }
  }
  cout << "fits file has " << interval << " rows" << endl;
  // Closing time:  
  //ft2file.close();
/*  if(m_digiFile && m_digiFile->IsZombie()) m_digiFile = 0;
  ...
  for (int iLoop = 0; iLoop < MaxEpuNumber; iLoop ++) {
    ...
    if (m_epuList.at(iLoop).m_lastDatagramEvent != -1) {
      ...
      if (lastReasonDataGram == enums::Lsf::Close::Full && completeRun) {
...
Bool_t ft2Verify::writeXmlFile(const char* fileName, int truncated) const {
  ...
  writeXmlIntervalSummary(elem, truncated);
  writeXmlFooter(elem);
...
void ft2Verify::writeXmlIntervalSummary(DomElement& node, int truncation) const {
  DomElement intSummary = AcdXmlUtil::makeChildNode(node,"intervalSummary");
